...
class LatencyState extends MusicBeatState
{
	...
	override function create()
	{
		...
		else
		{
			...
			#if android
			var tempText = new FlxText(0, 0, 700, "Press A \nWith the Beat", 60);
			#else
			...
			#if android
			tempText = new FlxText(0, 0, 1000, "Press B to input a value instead.", 20);
			#else
	...
	override function update(elapsed:Float)
	{
		...
		if (currState == "calibrate")
		{
			...
			if (FlxG.keys.justPressed.SPACE #if android || virtualPad.buttonA.justPressed #end)
			{
				trace("=================================");
		...
		else if (currState == "manual")
		{
			...
			if (FlxG.keys.pressed.DOWN #if android || virtualPad.buttonDown.pressed #end)
			{
				delta -= 1;
			}
