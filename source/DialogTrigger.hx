...
class DialogTrigger extends InteractableObject
{
	...
	public function UpdatePosition()
	{
		...
		fadeTween = FlxTween.tween(interactSprite, {y: interactSprite.y - 15}, 0.5, {ease: FlxEase.circOut, type: FlxTweenType.PINGPONG});
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (isColliding && isEnabled && !isActive)
			interactSprite.alpha = 1;
		else
			interactSprite.alpha = 0;
	...
	public override function OnCollideCallback():Bool
	{
		...
		if (FlxG.keys.anyJustPressed([SPACE]) #if android || justTouched #end)
		{
			OnActivate();
			isActive = true;
		}
	...
	{
		if (values != null)
			callbackParams = values.copy();
		if (values != null)
			stringsSequence = values.copy();
		if (cols != null)
			colorSequence = cols.copy();
		if (times != null)
			timingSequence = times.copy();
	}
	...
	public function GetCurrColor():FlxColor
	{
		if (colorSequence != null)
			return colorSequence[stringsSequence.length - callbackParams.length];
	...
	public function GetCurrTiming():Float
	{
		if (timingSequence != null)
			return timingSequence[stringsSequence.length - callbackParams.length];
		return 2;
	}
}
